namespace MediatorLib.Mapping
{
    ...
    {
        public Func<TSource, object?>? MapFromLambda { get; private set; }

            /// <summary>
            /// Defines a custom mapping function for a specific member of the destination type, allowing you to specify how to map the value from the source type.
            /// </summary>
            /// <param name="mapFrom">A function that defines how to map the value from the source type to the destination member.</param>
            public void MapFrom(Func<TSource, object?> mapFrom)
        {
            MapFromLambda = mapFrom;
        }
    }
        ...
        Action<IMemberOptions<TSource>> memberOptions)
    {
        ...
        {
            // This is only for DSL purposes; the generator reads the syntax, not executes this.
            return this;
        }
    }
}